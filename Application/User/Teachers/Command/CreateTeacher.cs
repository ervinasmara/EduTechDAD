...
namespace Application.User.Teachers.Command
{
    public class CreateTeacher
    {
        ...
        public class RegisterTeacherCommandValidator : AbstractValidator<RegisterTeacherCommand>
        {
            ...
            {
                RuleFor(x => x.TeacherDto.NameTeacher).NotEmpty().WithMessage("Name is required.");
                RuleFor(x => x.TeacherDto.BirthDate).NotEmpty().WithMessage("BirthDate is required.");
                RuleFor(x => x.TeacherDto.BirthPlace).NotEmpty().WithMessage("BirthPlace is required.");
                ...
                RuleFor(x => x.TeacherDto.Nip).NotEmpty().WithMessage("Nip is required.");
                RuleFor(x => x.TeacherDto.Gender).NotEmpty().WithMessage("Gender is required.")
                                      .InclusiveBetween(1, 2).WithMessage("Gender must be 1 for Male or 2 for Female.");
                RuleFor(x => x.TeacherDto.Username).NotEmpty().WithMessage("Username is required.");
                RuleFor(x => x.TeacherDto.Password)
                    .NotEmpty()
